...
from proofpress.legacy import artifact_provenance as proofpress_evidence
...
LEGACY_TOP_LEVEL_COMMANDS = {
    "snapshot", "log", "diff", "verify", "anchor", "ingest",
    "merge-plan", "merge", "merge-lineage", "identify", "export", "init",
    "sync", "blocks", "show", "policy", "inspect", "import", "clean",
    "capture", "provenance", "knowledge",
}


def _delegate(parser, subparsers, name, target, help_text):
    delegated = subparsers.add_parser(name, help=help_text, add_help=False)
    delegated.add_argument("delegate_args", nargs=argparse.REMAINDER)
    delegated.set_defaults(f=lambda a: target(a.delegate_args))


def main(argv=None):
    args = list(sys.argv[1:] if argv is None else argv)
    explicit_legacy = bool(args and args[0] == "legacy")
    if explicit_legacy:
        args = args[1:]
    elif args and args[0] in LEGACY_TOP_LEVEL_COMMANDS:
        print(
            f"warning: proofpress {args[0]} is a maintenance compatibility "
    ...
    if not explicit_legacy and args and args[0] in {"mcp", "hosted", "remote", "repo", "rd"}:
        ...
        delegates = {
            ...
            "rd": research_blueprint.main,
        }
        return delegates[args[0]](args[1:])
    ...
    if not explicit_legacy:
        ...
        _delegate(p, sub, "repo", repository.main, "build bounded single-repository evidence")
        _delegate(p, sub, "rd", research_blueprint.main, "compile a bounded R&D blueprint")
    a = p.parse_args(args)
    try:
        a.f(a)
