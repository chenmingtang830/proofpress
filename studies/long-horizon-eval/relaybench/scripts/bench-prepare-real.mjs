...
const result = await prepareRealPacket({
  ...
  candidatePath: args.candidate && path.resolve(args.candidate),
  stressFixturePath: args.stressFixture && path.resolve(args.stressFixture),
  coldBoundaryBefore: args.coldBoundaryBefore,
});
...
function parse(argv) { const out = {}; for (let i = 0; i < argv.length; i += 1) {
  ...
  else if (argv[i] === "--stress-fixture") out.stressFixture = argv[++i];
  else if (argv[i] === "--cold-boundary-before") out.coldBoundaryBefore = argv[++i];
