...
for (const task of taskNames) {
  let sender = await resolveSender(args.senderRoot ?? args.root, task);
  if (!sender && args.generateMissingSenders) sender = await generateSender(task);
  if (!sender) throw new Error(`missing reusable frozen sender for ${task}`);
...
async function generateSender(task) {
  ...
  for (let attempt = 1; attempt <= args.maxAttempts; attempt += 1) {
    ...
    try {
      await run("node", ["scripts/bench-run-real.mjs", "--phase", "prepare", "--packet", packet,
        "--output", output, "--track", args.track, "--env-file", args.envFile,
        "--authorize-real-calls"]);
      const resolved = await resolveSenderAt(output);
      if (!resolved) throw new Error(`generated sender is not reusable: ${output}`);
      process.stdout.write(`SENDER DONE ${args.track} ${task} explicit-attempt-${attempt}\n`);
      return output;
    } catch (error) {
      const failure = {
        schema_version: 1,
        classification: "INVALID_OPERATIONAL_SENDER_ATTEMPT",
        track_id: args.track,
        task,
        explicit_attempt: attempt,
        recorded_at: new Date().toISOString(),
      ...
      await fs.mkdir(output, { recursive: true });
      await fs.writeFile(path.join(output, "INVALID_ATTEMPT.json"), `${JSON.stringify(failure, null, 2)}\n`);
      process.stderr.write(`SENDER INVALID ${args.track} ${task} explicit-attempt-${attempt}: ${error.message}\n`);
    }
  }
  throw new Error(`exhausted ${args.maxAttempts} explicit sender attempts: ${args.track} ${task}`);
}

async function resolveSender(root, task) {
  const cell = path.join(root, "repeat-1", "clean", task);
  let entries;
  try { entries = await fs.readdir(cell, { withFileTypes: true }); } catch { return null; }
  ...
  for (const candidate of candidates.sort((a, b) => a === path.join(cell, "run") ? -1 : b === path.join(cell, "run") ? 1 : a.localeCompare(b))) {
    if (await resolveSenderAt(candidate)) return candidate;
  }
  return null;
}

async function resolveSenderAt(candidate) {
  ...
  return ["POLICY_GATE_COMPLETE", "READY_FOR_LAB_EVALUATION", "COMPLETE"].includes(state.status)
    && ["S1", "S2", "S3"].every((id) => stages.some((stage) => stage.stage_id === id));
}
...
function parse(argv) {
  const out = { arms: "clean,stress", repeats: "1,2,3", maxAttempts: 3, generateMissingSenders: false };
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (key === "--generate-missing-senders") out.generateMissingSenders = value === "true";
