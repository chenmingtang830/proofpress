...
test("real packet freezes paired conditions and a human checkpoint without model calls", async () => {
  ...
  const review = JSON.parse(await fs.readFile(path.join(output, "HUMAN_REVIEW.template.json")));
  assert.equal(review.signed_at, null);
});
