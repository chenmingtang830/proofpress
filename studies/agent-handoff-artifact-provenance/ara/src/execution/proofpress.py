...
def valid_capsule_discovery(value):
    """Accept the prior safe discovery tag so portable capsules can upgrade.

    ``latest`` was emitted by earlier Proofpress releases.  It names the same
    approved package and URL as the current ``next`` tag, so treating it as a
    tamper signal would make a valid linear portable history impossible to
    extend after a tool upgrade.
    """
    if value == CAPSULE_DISCOVERY:
        return True
    legacy = dict(CAPSULE_DISCOVERY)
    legacy["dist_tag"] = "latest"
    return value == legacy
...
def cmd_snapshot(a):
    ...
    if meta["policy"] == "portable" and capsule is not None:
        ...
        # A prior snapshot can have reached the local ledger before an older
        # portable capsule was rejected by a newly upgraded client.  If the
        # ledger already contains this exact body, repair only the carrier;
        # creating another same-body event would misstate the history.
        latest = latest_for(a.file)
        if ("body_mismatch" in cap_state and latest and
                latest.get("artifact") == a.file and
                capsule.get("head") != latest.get("version")):
            version = read_version(latest["_commit"])
            current_v = {
                "artifact": a.file,
                "artifact_id": meta["artifact_id"],
                "blocks": assign_ids(
                    parse_blocks(body, carrier_for_path(a.file)), version),
            }
