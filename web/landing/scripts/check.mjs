...
const requirements = [
  ...
  [app.includes('import posts from "./content/post-index.json"') && app.includes('href={`/blog/${item.slug}`}') && !app.includes(".slice(0, 3)") && app.includes('const featuredSlug = "agents-create-a-new-knowledge-layer"'), "all imported articles, with a pinned lead article, link to native blog pages without bundling full article bodies"],
