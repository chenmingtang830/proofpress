...
const requirements = [
  ...
  [app.includes("The Intelligence Ledger for AI research teams.") && app.includes("AI research labs") && app.includes("Post-training teams") && app.includes("Benchmark &amp; evaluation teams"), "the research audience and target team types are explicit"],
  [app.indexOf('className="productClosingStatement"') > app.indexOf('className="productMechanism"') && app.indexOf('className="productClosingStatement"') < app.indexOf('id="ledger-system-note"') && app.indexOf('className="productClosingStatement"') < app.indexOf('className="verificationFlow"') && app.includes("Less repeated work. More time for new research.") && !app.includes('className="productOutcomes"') && !app.slice(0, app.indexOf('className="why"')).includes("RSI"), "the concise product promise sits below the ledger and above its illustrative note without adding another explanatory block"],
  [!app.includes('className="heroFoot"') && app.includes("Explore open source on GitHub"), "hero retains the open-source action without redundant attribute labels"],
  ...
  [app.includes('className="mobileNav"') && app.includes("View evidence and reuse scope") && app.includes("Human Approval") && app.includes("learningCycle") && app.includes("Review what changes.") && !app.includes('className="loopReturn"'), "mobile disclosure and the separate learning-loop section preserve the complete research story without a redundant return strip"],
  ...
  [app.includes("<small>by <span>Only Then Labs</span></small>") && !app.includes("Proofpress by <span>Only Then Labs</span>.") && app.includes("The Intelligence Ledger for agent-native research teams.") && app.includes("<h3>Applied AI teams</h3>"), "the landing keeps the concise Only Then Labs attribution and its aligned research-team ICP"],
  ...
  [app.includes('import posts from "./content/post-index.json"') && app.includes('href={`/blog/${item.slug}`}') && !app.includes(".slice(0, 3)") && app.includes('const featuredSlug = "atom-bit-commit-agent-knowledge"'), "all imported articles, with the current lead article pinned, link to native blog pages without bundling full article bodies"],
  [app.includes("Human Approval authorizes reuse") && app.includes("Build on trusted findings.") && app.includes("Learn from new work.") && app.includes("Review what changes.") && !app.includes("UPDATE TRUST WEIGHTS"), "the learning loop connects governed context, downstream claims, outcomes, and evidence-led refinement without implying automatic trust weighting"],
  [chart.includes("Illustrative model"), "conceptual chart is not presented as measured evidence"],
  ...
  [html.includes('property="og:url" content="https://proofpress.dev/"') && html.includes('property="og:image" content="https://proofpress.dev/og-proofpress.png?v=20260924"') && html.includes('property="og:image:width" content="1200"') && html.includes('property="og:image:height" content="630"') && html.includes('property="og:image:alt"'), "complete Open Graph image metadata is declared"],
  [html.includes('name="twitter:card" content="summary_large_image"') && html.includes('name="twitter:title"') && html.includes('name="twitter:description"') && html.includes('name="twitter:image" content="https://proofpress.dev/og-proofpress.png?v=20260924"') && html.includes('name="twitter:image:alt"'), "complete Twitter large-image metadata is declared"],
  ...
  [html.includes("Proofpress — Make every discovery count") && html.includes("Preserve trusted findings"), "page and social text metadata use the current research positioning"],
];
