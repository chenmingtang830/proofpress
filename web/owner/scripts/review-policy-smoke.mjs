...
try {
  ...
  page.on('dialog',()=>{throw new Error('Native dialog must not be used for model review');});
  ...
  await page.goto(`${data.base}/review?claim_id=${cid}&view=full`);
  ...
  const blockedReceipt = await (await page.request.get(`${data.base}/owner/api/claims/${cid}`)).json();
  assert.equal(blockedReceipt.result.evaluation.eligible, false);
  ...
  await page.getByLabel('Model review',{exact:true}).selectOption('off');
  ...
  await page.reload();
  assert.equal(await page.getByLabel('Model review',{exact:true}).inputValue(),'off');
