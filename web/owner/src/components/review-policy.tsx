  ...
  return <Card className="reviewPolicy" aria-labelledby="reviewPolicyTitle">
    ...
    {!settings ? <CardContent><p role="status">Loading policy…</p></CardContent> : <CardContent><form onSubmit={save}>
      ...
      <fieldset className="autoAdmissionFieldset"><legend>Automatic approval</legend>
        <Label className="policyCheck autoAdmissionToggle"><Checkbox checked={settings.auto_admit_new_claims === true} disabled={!settings.auto_admit_new_claims && (settings.mode !== "automatic" || !settings.require_judge || !settings.criteria.trim() || !settings.external_consent || !hasProviderCredential)} onCheckedChange={checked=>change("auto_admit_new_claims",checked === true)} /><span><b>Approve qualifying new claims automatically</b><small>Applies to new claims across this workspace, including high-risk claims.</small></span></Label>
        {settings.auto_admit_new_claims ? <div className="policyWarning autoAdmissionWarning" role="note"><strong>Automatic approval will take effect when you save.</strong><p>A claim qualifies when it was proposed after the current policy was saved, current checks pass, there is no unresolved conflict or invalid dependency, and the current model recommendation is “Evidence supported.” An existing owner decision, a policy change during review, or missing, rejected, escalated, or failed model advice prevents automatic approval. Qualifying claims will not receive per-claim human approval. Model recommendations are not calibrated probabilities.</p></div> : <><p className="policyHint">Only claims proposed after the current policy is saved can qualify. Current checks must pass, the model must recommend “Evidence supported,” and there must be no unresolved conflict or invalid dependency.</p><p className="policyHint">To enable it, run model review automatically, require supporting advice, add criteria, provide a valid API key, and allow provider processing.</p></>}
      </fieldset>
