...
export function ReviewPolicy({csrf, api, onSaved}: any) {
  ...
  return <section className="reviewPolicy" aria-labelledby="reviewPolicyTitle">
    <div className="policyHeading"><div><h2 id="reviewPolicyTitle">Review policy</h2><p>Choose when LM advice is required, then configure how the review runs.</p></div>{record?.version > 0 && <span>Policy v{record.version}</span>}</div>
    ...
    {!settings ? <p role="status">Loading policy…</p> : <form onSubmit={save}>
      <fieldset className="approvalRequirement"><legend>Approval requirement</legend>
        <p className="policySectionIntro">Human Approval always makes the final decision. Choose whether supporting LM advice must be present first.</p>
        <div className="approvalChoices" role="radiogroup" aria-label="Approval requirement">
          <label className={!settings.require_judge ? "selected" : ""}><input type="radio" name="approval-requirement" checked={!settings.require_judge} onChange={()=>change("require_judge",false)} /><span><b>Human can decide anytime</b><small>LM advice stays advisory and never blocks Approve.</small></span></label>
          <label className={settings.require_judge ? "selected" : ""}><input type="radio" name="approval-requirement" checked={settings.require_judge} disabled={settings.mode==="off"} onChange={()=>change("require_judge",true)} /><span><b>Require supporting LM advice</b><small>Approve stays unavailable until current LM advice supports the evidence.</small></span></label>
        </div>
        {settings.mode==="off" && <p className="policyWarning">Turn on LM review below before requiring supporting advice.</p>}
      </fieldset>
      <fieldset><legend>LM review</legend>
        ...
        <details className="providerCredential"><summary>Provider credential</summary><div className="providerCredentialBody"><label>API key<input type="password" autoComplete="new-password" value={apiKey} disabled={removeKey} placeholder={record.credential.configured ? `Saved key ending in ${record.credential.last_four || "••••"}` : "Paste a provider key"} onChange={e=>setApiKey(e.target.value)} /></label>
          ...
          {!record.credential.storage_ready && <p className="policyWarning">Secure credential storage is unavailable on this deployment.</p>}</div>
        </details>
      </fieldset>
