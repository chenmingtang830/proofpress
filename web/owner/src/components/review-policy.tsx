    ...
    {!settings ? <CardContent><p role="status">Loading policy…</p></CardContent> : <CardContent><form onSubmit={save}>
      ...
      <fieldset><legend>Model provider</legend>
        <div className="policyFields three">
          ...
          <label>Model{settings.provider==="custom" || selectedProvider?.editable_model ? <><input list={modelOptions.length?"provider-model-options":undefined} value={settings.model} placeholder="provider/model-name" onChange={e=>change("model",e.target.value)} />{modelOptions.length>0 && <datalist id="provider-model-options">{modelOptions.map((model:string)=><option key={model} value={model}/>)}</datalist>}</> : <select aria-label="Model" value={settings.model} onChange={e=>change("model",e.target.value)}>{modelOptions.map((model:string)=><option key={model} value={model}>{model}</option>)}</select>}</label>
