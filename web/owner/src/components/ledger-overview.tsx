...
export function LedgerOverview({rows, nodes, edges, onChoose}: any) {
  ...
  if (!rows.length) return <p className="emptyState">No claims in this view.</p>;
  return <div className={`globalLineage${mobileGraph ? " showMobileGraph" : ""}`}>
    <div className="globalToolbar"><Label>Scope <NativeSelect value={scope} onChange={e=>{setScope(e.target.value);setLimit(6);setSourceLimit(12);setSource(null);}}><option value="">All scopes</option>{scopes.map(s=><option key={s}>{s}</option>)}</NativeSelect></Label><span>{filtered.length} claims in this view</span>{source && <Button variant="outline" onClick={()=>setSource(null)}>Clear evidence selection</Button>}</div>
    ...
    <div className="mobileLineageList">{shown.map((r:any) => <article key={r.id}>
      <Badge state={r.state} />
      <Button variant="ghost" size="content" className="mobileClaim" onClick={() => onChoose(r.id)}>{claimDisplayTitle(r)}</Button>
      <p>{r.scope || "Workspace"}</p>
      <Disclosure><DisclosureTrigger>Supporting evidence ({support.filter((e:any) => e.to === r.id).length})</DisclosureTrigger><DisclosureContent>
    ...
    <div className="graphScroll" tabIndex={0} aria-label="Global evidence and claim graph">
      <div className="globalGraph" style={{height}}>
        <div className="globalColumns"><span>Evidence</span><span>Claims</span></div>
        ...
        {sources.map((id,i)=>{const n=nodes.find((n:any)=>n.id===id);return <Button variant="ghost" size="content" key={id} className="globalNode source" style={{top:64+i*126}} aria-pressed={source===id} onClick={()=>setSource(source===id?null:id)}><strong>{n?.label || id}</strong><small>{id} · {support.filter((e:any)=>e.from===id).length} linked claims</small></Button>;})}
        {shown.map((r:any,i:number)=><Button variant="ghost" size="content" key={r.id} className="globalNode claimNode" data-state={r.state} style={{top:64+i*126,opacity:related(r.id)?1:.35}} onClick={()=>onChoose(r.id)}><small>{r.state.replaceAll("_"," ")} · {r.created_at ? new Date(r.created_at).toLocaleDateString() : "Time not recorded"}</small><strong>{claimDisplayTitle(r)}</strong></Button>)}
      </div>
    </div>
