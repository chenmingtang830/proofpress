  ...
  return <div className="lineageDiagram">
    <div className="graphScroll" tabIndex={0} aria-label="Lineage graph; scroll horizontally on small screens">
      <div className="graphPlane" style={{height}}>
        ...
        <svg width="100%" height={height} viewBox={`0 0 920 ${height}`} preserveAspectRatio="none" aria-hidden="true">{evidence.map((_:any,i:number) => <path key={i} d={`M 262 ${134+i*138} C 305 ${134+i*138}, 295 ${center}, 338 ${center}`} />)}<path className={available ? "" : "excluded"} d={`M 582 ${center} C 620 ${center}, 622 ${center}, 662 ${center}`} /></svg>
