...
export function LineageGraph({receipt, available, evidenceNames, selection, onSelect}: any) {
  ...
  const applicability = receipt.conclusion.applicability || {};
  const boundary = receipt.conclusion.scope || applicability.title || applicability.description || "No reuse boundary recorded";
  const contextTitle = receipt.conclusion.scope ? `Scope: ${receipt.conclusion.scope}` : boundary;
  ...
  return <div className="lineageDiagram">
    <div className="graphScroll" tabIndex={0} aria-label="Lineage graph; scroll horizontally on small screens">
      <div className="graphPlane" style={{height}}>
        ...
        {node("conclusion",338,center-70,receipt.conclusion.statement,receipt.state.replaceAll("_"," "),[boundary, receipt.conclusion.created_at && new Date(receipt.conclusion.created_at).toLocaleString()].filter(Boolean).join(" · "),`conclusion ${tone}`)}
