  ...
  return <div className="lineageDiagram">
    <div className="graphScroll" tabIndex={0} aria-label="Lineage graph; scroll horizontally on small screens">
      <div className="graphPlane" style={{height}}>
        ...
        {node("conclusion",338,center-70,receipt.conclusion.statement,receipt.state.replaceAll("_"," "),[receipt.conclusion.scope, receipt.conclusion.created_at && new Date(receipt.conclusion.created_at).toLocaleString()].filter(Boolean).join(" · "),`conclusion ${tone}`)}
        {node("context",662,center-70,available ? `Scope: ${receipt.conclusion.scope || "Workspace"}` : receipt.state === "admitted" ? "Not eligible in this view" : `Not reusable: ${receipt.state.replaceAll("_"," ")}`,"Reuse boundary", available ? `Approved by ${receipt.review?.reviewer || "actor not recorded"}` : Object.entries(receipt.evaluation?.checks || {}).filter(([,ok])=>!ok).map(([name])=>name.replaceAll("_"," ")).join(", ") || (receipt.state === "admitted" ? "Check scope and actor eligibility" : "Human approval required"),`context ${tone}`)}
      </div>
    </div>
