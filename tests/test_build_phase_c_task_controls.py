...
class PhaseCTaskControlTests(unittest.TestCase):
    def write_tasks(self, root: Path):
        identifiers = MANIFEST["held_out_task_ids"]
    ...
    def test_compiles_disjoint_executor_and_grader_controls(self):
        ...
        self.assertEqual(len(rubrics["rubrics"]), 7)
    ...
    def test_rejects_a_development_task_in_the_heldout_custody_slice(self):
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp); self.write_tasks(root)
            task_id = MANIFEST["development_task_ids"][0]
            value = {"task_id": task_id, "prompt": "development prompt",
                     "expected_output": "message_in_console", "rubric": [{"id": "development"}]}
            (root / "development.json").write_text(json.dumps(value))
            with self.assertRaisesRegex(ValueError, "do not match frozen panel"):
                builder.build(manifest=MANIFEST, task_root=root)
