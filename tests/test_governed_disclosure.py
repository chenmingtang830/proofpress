...
class GovernedDisclosureTests(unittest.TestCase):
    def setUp(self):
        ...
        evidence = kernel_ops.import_evidence_v2(str(self.source))["evidence"][0]
        self.visible = kernel_ops.propose_v2("Liability cap is one times annual fees", [evidence],
                                            "matter-1", "agent:proposer")["claim"]["id"]
        kernel_ops.review_v2(self.visible, "admit", "human:reviewer")
...
""".replace("MODE", repr(mode)))
    ...
    def _submit_key(self, dry):
        return kernel_ops.digest({"packet_digest": dry["packet_digest"],
    ...
    def test_governed_context_contains_admitted_workspace_knowledge(self):
        packet = kernel_ops.disclose_v1("What is the liability cap?", "agent:executor", "matter-1")
    ...
    def test_novel_discovery_is_not_governed_and_never_mutates_ledger(self):
        manifest, _ = self._manifest(); head = kernel_ops.v2_head()
        packet = kernel_ops.disclose_v1("What indemnity carveout applies?", "agent:executor", "matter-1",
                                       corpus_manifest=str(manifest), sidecar=self._sidecar())
        self.assertEqual(kernel_ops.v2_head(), head)
    ...
    def test_source_custody_and_locator_fail_closed(self):
        ...
        with self.assertRaisesRegex(ValueError, "outside the corpus"):
            kernel_ops.disclose_v1("indemnity carveout", "agent:executor", "matter-1",
    ...
    def test_assimilation_is_dry_run_then_explicit_non_admitting_submit(self):
        ...
        dry = kernel_ops.assimilate_v1(packet, "agent:executor", "matter-1",
                                      corpus_manifest=str(manifest), recommender=recommender)
        self.assertEqual(dry["schema_version"], kernel_ops.ASSIMILATION_SCHEMA)
        self.assertFalse(dry["submitted"])
        self.assertEqual(kernel_ops.v2_head(), head)
        key = kernel_ops.digest({"packet_digest": dry["packet_digest"],
                                "ledger_head": dry["ledger_head"], "actor": "agent:executor",
                                "scope": "matter-1", "receipts": dry["receipt_digests"],
                                "config_digest": dry["config_digest"]})
        submitted = kernel_ops.assimilate_v1(packet, "agent:executor", "matter-1",
                                             receipt_digests=dry["receipt_digests"],
                                             gap_ids=dry["gap_ids"], expected_head=head,
                                             submit=True, idempotency_key=key,
                                             corpus_manifest=str(manifest), recommender=recommender)
        self.assertTrue(submitted["submitted"])
        self.assertTrue(any(e["type"] == "assimilation_submitted" for e in kernel_ops.v2_events()))
        self.assertFalse(any(e["type"] == "claim_admitted" and
                             e.get("subject_ref") != self.visible
                             for e in kernel_ops.v2_events()))
        replay = kernel_ops.assimilate_v1(packet, "agent:executor", "matter-1",
                                          submit=True, idempotency_key=key,
                                          corpus_manifest=str(manifest), recommender=recommender)
    ...
    def test_assimilation_submit_can_create_only_an_unresolved_candidate(self):
        ...
        dry = kernel_ops.assimilate_v1(packet, "agent:executor", "matter-1",
                                      corpus_manifest=str(manifest), recommender=recommender)
        submitted = kernel_ops.assimilate_v1(
        ...
        self.assertEqual(kernel_ops.v2_state(projection, projection["claims"][candidate_id]), "needs_review")
    ...
    def test_conflict_submit_adds_unresolved_candidate_and_relation_without_overwrite(self):
        ...
        kernel_ops.import_evidence_v2(str(prior_path))
        projection = kernel_ops.v2_projection()
        prior_evidence = next(row["id"] for row in projection["evidence"].values()
                              if row.get("kind") == "retrieval_evidence")
        prior_claim = kernel_ops.propose_v2("Legacy payment term", [prior_evidence], "matter-1",
                                           "agent:proposer")["claim"]["id"]
        kernel_ops.review_v2(prior_claim, "admit", "human:reviewer")
        packet = kernel_ops.disclose_v1(
            "What indemnity carveout applies?", "agent:executor", "matter-1",
            corpus_manifest=str(manifest), sidecar=self._sidecar())
        recommender = lambda request: {
        ...
        submitted = kernel_ops.assimilate_v1(
            packet, "agent:executor", "matter-1", expected_head=dry["ledger_head"], submit=True,
            idempotency_key=self._submit_key(dry), corpus_manifest=str(manifest), recommender=recommender)
        projection = kernel_ops.v2_projection(); candidate_id = submitted["candidate"]["id"]
        self.assertEqual(kernel_ops.v2_state(projection, projection["claims"][candidate_id]), "needs_review")
        self.assertEqual(projection["claims"][prior_claim]["statement"], "Legacy payment term")
        relation = projection["relations"][submitted["relations"][0]["id"]]
        self.assertEqual(relation["type"], "contradicts")
        self.assertEqual(kernel_ops.relation_state(projection, relation), "needs_review")
    ...
    def test_assimilation_requires_fresh_custody_and_gap_binding(self):
        ...
        with self.assertRaisesRegex(ValueError, "corpus manifest"):
            kernel_ops.assimilate_v1(packet, "agent:executor", "matter-1", recommender=recommender)
        with self.assertRaisesRegex(ValueError, "gap id"):
            kernel_ops.assimilate_v1(packet, "agent:executor", "matter-1", gap_ids=["gap_missing"],
                                    corpus_manifest=str(manifest), recommender=recommender)
