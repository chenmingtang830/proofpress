...
function runPython(args, options) {
  ...
  const cli = path.join(sourceRoot, "proofpress", "cli.py");
  if (!fs.existsSync(cli)) {
    console.error(`proofpress: bundled Python package not found: ${cli}`);
    return 1;
  }
  ...
  for (const [command, prefix] of pythonCandidates()) {
    const result = spawnSync(command, [...prefix, "-m", "proofpress.cli", ...args], {
      cwd: options.cwd,
      stdio: "inherit",
